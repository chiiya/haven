...
/**
 * Create a named google analytics tracker. Useful when having multiple trackers.
 */
const createNamedTracker = (options: GoogleAnalyticsOptions) => {
  (<any>window).ga('create', options.id, 'auto', options.name);
  if (options.anonymizeIp !== false) {
    (<any>window).ga(`${options.name}.set`, 'anonymizeIp', true);
  }
  (<any>window).ga(`${options.name}.send`, 'pageview');
};
...
const createDefaultTracker = (options: GoogleAnalyticsOptions) => {
  (<any>window).ga('create', options.id, 'auto');
  if (options.anonymizeIp !== false) {
